...
import com.example.movieapp.domain.local.usecase.GetMovieUseCase
...
class DownloadViewModel @Inject constructor(
    ...
    fun getMovies() = viewModelScope.launch {
        getMoviesUseCase().collect { movies ->
            Log.i("INFOTESTE", "VIEWMODEL getMovies() === Movies: ${movies.size}")
    ...
    fun searchMovie(search: String) = viewModelScope.launch {
        val newList = _movieList.value?.filter {
